...
# run_name = 'fusion-freeze-lr-1e-6-2layer-long-bs-64'
# epoch = '75'
    ...
    with torch.no_grad():
        ...
        for i, timepoint in enumerate(time_points):
            ...
            if len(np.unique(y_true)) >= 2:  # 确保有正负样本
                ...
                timepoint_result['auprc'] = ap
                PrecisionRecallDisplay.from_predictions(y_true, y_score)
                plt.title(f"P-R Curve (Time ≤ {timepoint}h)")
        ...
        for i, bin_name in enumerate(bin_names):
            ...
            if len(np.unique(y_true_bin)) > 1:
                ...
                bin_result['AUPRC'] = ap
                PrecisionRecallDisplay.from_predictions(y_true_bin, y_prob_bin)
                plt.title(f"P-R Curve ({bin_name})")
        ...
        ap_overall = average_precision_score(y_events, y_prob)
        print('AUPRC:', ap_overall)
        PrecisionRecallDisplay.from_predictions(y_events, y_prob)
        plt.title("Overall P-R Curve (Binary Classification)")
        plt.savefig(os.path.join(save_info_dir, "overall_pr_curve_binary.png"), bbox_inches='tight', dpi=300)
        plt.close()
