...
for _, row in tqdm(patient_df.iterrows(), total=len(patient_df)):
    ...
    for main_header_file in main_header_files:
        ...
        for line in lines[1:]:
            ...
            try:
                ...
                offset_samples = int(fs * OFFSET_HOURS * 3600)
                ...
                # 全为0
                if (record.p_signal.std(axis=0) == 0).any():
                    continue
    ...
    if len(candidates) > 0:
        ...
        summary_rows.append({
            ...
            'ecg_time': best['segment_time'].isoformat(),
            'sig_name': best['record'].sig_name,
            'fs': best['fs']
        })
    else:
        print(f"[Waring] Cannot find candidate file in {subj_path}")
